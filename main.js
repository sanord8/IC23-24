...
// TODO: Add css
const css = { start: "start", finish: "finish", active: "active",water: "water",landscape: "landscape" };
...
GraphSearch.prototype.cellClicked = function ($cell) {
    //------------------initialization------------------
    let clickedCell = this.nodeFromElement($cell);
    

    if ($end.hasClass(css.water) || $end.hasClass(css.start)) { return; }
    ...
    let path = this.search(this.graph, start, end, water, landscape, {
...
document.querySelectorAll("#icons .col").forEach(elem => {
    elem.addEventListener("click", e => {
        document.getElementsByTagName("body")[0].style.cursor = `url('images/${e.target.innerHTML}.cur'), auto`;
    });
});